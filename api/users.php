...
function handle_get(): void {
    ...
    $stmt = $pdo->prepare('SELECT id, email, role, barangay, created_at, archived, last_activity FROM users WHERE archived = ? ORDER BY role DESC, created_at ASC');
    $stmt->execute([$archived]);
    $users = $stmt->fetchAll();
    
    $timeStmt = $pdo->query('SELECT NOW() as server_time');
    $currentTime = $timeStmt->fetch()['server_time'];
    ...
    foreach ($users as &$user) {
        if ($user['last_activity']) {
            ...
            if ($minutesAgo < 1) {
                $user['activity_status'] = 'Active now';
            } elseif ($minutesAgo < 60) {
                $user['activity_status'] = "Active {$minutesAgo} minute" . ($minutesAgo > 1 ? 's' : '') . " ago";
            } elseif ($minutesAgo < 1440) { // Less than 24 hours
                $hoursAgo = floor($minutesAgo / 60);
                $user['activity_status'] = "Active {$hoursAgo} hour" . ($hoursAgo > 1 ? 's' : '') . " ago";
            } else {
                $daysAgo = floor($minutesAgo / 1440);
                $user['activity_status'] = "Active {$daysAgo} day" . ($daysAgo > 1 ? 's' : '') . " ago";
            }
        } else {
...
function handle_delete(): void {
    ...
    try {
        if ($email !== '') {
            $stmt = $pdo->prepare('SELECT id, role, last_activity FROM users WHERE email = ?');
            $stmt->execute([$email]);
        } else {
            $stmt = $pdo->prepare('SELECT id, role, last_activity FROM users WHERE id = ?');
            $stmt->execute([$id]);
        }
        $user = $stmt->fetch();
        if (!$user) {
